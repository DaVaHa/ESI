'''
This script will extract the short interest notifications for Euronext Amsterdam.
'''
...
for f in os.listdir('/home/daniel/Desktop/Scripts/_ShortNotifications/'):
    file_name = 'XBRU_{}'.format(TimeStamp())
    if f.startswith(file_name):
        files.append(f)
file = files[0]


Logging("Loading Excel file...\n")
...
for sht in sheets:
    ...
    try:
        ...
        # find correct starting row
        for i in range(10):
            try:
                if re.search('HOLDER', df.iloc[i,0].upper()):
                    j = i + 1
                    break
            except:
                pass
        df = pd.read_excel(xlsx, sheetname=sht, skiprows=j)
        
        # renaming columns
        for clm in df.columns:
            if re.search('UNWINDING', clm.upper()):
                df.rename(columns={clm : 'UNWINDING/CHANGE'}, inplace=True)
            elif re.search('ISIN', clm.upper()):
                df.rename(columns={clm : 'ISIN'}, inplace=True)
            elif re.search('%', clm) or re.search('CAPITAL', clm.upper()):
                df.rename(columns={clm : 'INTEREST'}, inplace=True)
            elif re.search('DATE', clm.upper()):
                df.rename(columns={clm : 'POSITION_DATE'}, inplace=True)
            elif re.search('HOLDER', clm.upper()):
                df.rename(columns={clm : 'HOLDER'}, inplace=True)
            elif re.search('ISSUER', clm.upper()):
        ...
        df['POSITION_DATE'] = df['POSITION_DATE'].apply(lambda x : datetime.datetime.strftime(x, "%Y%m%d"))

        if 'UNWINDING/CHANGE' in df.columns:
            df['UNWINDING/CHANGE'] = df['UNWINDING/CHANGE'].apply(lambda x : datetime.datetime.strftime(x, "%Y%m%d"))

        # starting index from 1 instead of 0
        df.index += 1
        
        # creating table in database
        table_name = sht.replace(' ', '_')
