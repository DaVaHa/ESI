...
def GraphSI(issuer_tuple, mic):
    ...
    fig, ax1 = plt.subplots(sharex=True, figsize=(8,6))
    fig.patch.set_facecolor((1,1,1))
    ax1.set_xbound(lower=2012)
    ...
    #ax1.grid(False)
    if max_si > 15: ax1.set_ylim([-0.2,20.5])
    elif max_si > 10 : ax1.set_ylim([-0.2,15.5])
    elif max_si > 5 : ax1.set_ylim([-0.2,10.5])
    else: ax1.set_ylim([-0.2,5.5])
    
    color_price = [0,114/255,178/255]
    ax2 = ax1.twinx()
    ax2.set_xbound(lower=2012)
    ax2.plot(combi['Price'], linewidth=1.3, color=color_price)
    #ax2.set_ylabel('Price')
    ax2.grid(False)
    
    # graph
    #plt.tick_params(axis='both', which='major', labelsize=12)
    plt.title('{} ({})'.format(title_name, mic), fontsize=24)
    plt.suptitle('Short Interest vs Stock Price', y=0.88, fontsize=12)
    plt.show()
...
def GetIssuersMIC(mic):
    ...
    # get all issuers
    cur.execute('''SELECT DISTINCT ISSUER, PRETTY_NAME, GRAPH_NAME, QUANDL_CODE FROM Issuers WHERE MIC="{}" ORDER BY ISSUER LIMIT 5;'''.format(mic))
